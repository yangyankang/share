    ...
    >
      ...
      <el-table-column label="操作" align="center" fixed="right" min-width="300" show-overflow-tooltip>
        <span class="table-handle-btns" slot-scope="{row}">
          ...
          <span
            class="handle-btn success"
            @click="openYuYue(row)"
            v-if="['IDENTIFY', 'END'].indexOf(row.flowStatus) === -1"
          >
            分派
          </span>
          <span
            ...
            v-if="['ASSIGNED', 'LEAVE', 'REACHED'].indexOf(row.flowStatus) !== -1 && row.status !== 0"
          >
            核销
          </span>
          <span
            class="handle-btn danger"
            @click="openRepairOrder(row)"
            v-if="['IDENTIFY', 'END'].indexOf(row.flowStatus) !== -1 && row.status !== 0"
          >
            维修
          </span>
          <span
            class="handle-btn warn"
...
import ClosureDialog from './closure'
